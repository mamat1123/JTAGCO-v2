...
export const InactiveCompaniesPage = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["inactiveCompanies", monthsBack, user_id, page, limit, sortBy],
    queryFn: () =>
      companyAPI.getInactiveCompanies({
        months: Number(monthsBack),
        page,
        limit,
        sortBy,
        ...(user_id && user_id !== "all" && { user_id }),
      }),
  });

  const { data: stateData, error: stateError } = useQuery({
    queryKey: ["inactiveCompanyStats", monthsBack, user_id, sortBy],
    queryFn: () => companyAPI.getInactiveCompanyStats({
      months: Number(monthsBack),
      ...(user_id && user_id !== "all" && { user_id }),
      sortBy,
    }),
  ...
  const hasMore = data
    ? allCompanies.length < (stateData?.total_companies || 0)
    : false;
  ...
  const notHaveEventPercentage = stateData?.never_updated
    ? ((stateData?.never_updated / stateData?.total_companies) * 100).toFixed(2)
    : 0;
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">
            บริษัทที่ไม่ได้ใช้งาน
          </h1>
          <p className="text-gray-600">
            บริษัทที่ไม่มีกิจกรรมอัปเดตในช่วงเวลาที่เลือก
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stateData?.total_companies || 0}
              </div>
              <p className="text-xs text-muted-foreground">
                ไม่มีการใช้งานในช่วง {monthsBack} เดือนที่ผ่านมา
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                สัดส่วนบริษัทที่ไม่เคยมี event
              </CardTitle>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ปรับแต่งมุมมองของบริษัทที่ไม่ได้ใช้งาน
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <div className="space-y-2">
                ...
                <Select value={monthsBack} onValueChange={handleMonthsBackChange}>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="last_event_updated_at">
                      กิจกรรมล่าสุด (ใหม่ไปเก่า)
                    </SelectItem>
                    <SelectItem value="credit">
                      เงื่อนไขเครดิต (มากไปน้อย)
                    </SelectItem>
                    <SelectItem value="total_employees">
                      พนักงาน (มากไปน้อย)
                    </SelectItem>
                  </SelectContent>
                </Select>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              พบ {stateData?.total_companies || 0}{" "}
              บริษัทที่ไม่มีกิจกรรมอัปเดตในช่วง {monthsBack} เดือนที่ผ่านมา
            </CardDescription>
          </CardHeader>
          <CardContent>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <span className="font-medium">
                            {(company.totalEmployees || 0).toLocaleString()}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          <CreditCard className="h-4 w-4 text-muted-foreground" />
                          <span className="font-medium">
                            {formatCreditDays(company.credit || 0)}
                          </span>
                        </div>
                      </TableCell>
                    </TableRow>
